...
class InstancesDocBuilder(object):
    ...
    def _build_instance_library_link(self, instanceReferenceList:List[Dict]) -> str:
        ...
        if instance_example_type in ["accessibility", "anatomicalAtlas", "brainAtlas", "contentType", "commonCoordinateSpace", "commonCoordinateFramework", "license"]:
            link_dir = os.path.join(self.readthedocs_url, self.version, "instance_libraries")
            page_heading = instance_type_plural
        elif instance_example_type in ["anatomicalAtlasVersion", "brainAtlasVersion", "commonCoordinateSpaceVersion", "commonCoordinateFrameworkVersion", "parcellationEntity",]:
            link_dir = os.path.join(self.readthedocs_url, self.version, "instance_libraries", instance_type_plural)
    ...
    def build(self):
        for instancelib_docu_relative_path, instances_absolute_paths in self.relative_paths_for_instancelib_docu.items():
            ...
            if instancelib_docu_relative_path.split("/")[0] in ["anatomicalAtlases", "brainAtlases", "commonCoordinateSpaces", "commonCoordinateFrameworks", "brainAtlasVersions", "anatomicalAtlasVersions", "commonCoordinateSpaceVersions", "commonCoordinateFrameworkVersions"]:
