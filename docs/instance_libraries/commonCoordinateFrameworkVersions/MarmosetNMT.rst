######################################################
CommonCoordinateFrameworkVersions: MarmosetNMT library
######################################################

Related schema specification: `CommonCoordinateFrameworkVersion <https://openminds-documentation.readthedocs.io/en/latest/schema_specifications/SANDS/atlas/commonCoordinateFrameworkVersion.html>`_

------------

------------

MarmosetNMT_v1
--------------

.. admonition:: metadata sheet

   :@context: @vocab: <https://openminds.om-i.org/props/>
   :@id: https://openminds.om-i.org/instances/commonCoordinateFrameworkVersion/MarmosetNMT_v1
   :@type: https://openminds.om-i.org/types/CommonCoordinateFrameworkVersion
   :abbreviation: MarmosetNMT
   :accessibility: `directVirtualOpenAccess <https://openminds-documentation.readthedocs.io/en/latest/instance_libraries/accessibilities.html#directvirtualopenaccess>`_
   :anatomicalAxesOrientation: `LPI <https://openminds-documentation.readthedocs.io/en/latest/instance_libraries/terminologies/anatomicalAxesOrientation.html#lpi>`_
   :description: The Nencki-Monash (NM) template v1.0 (2020) represents a computational morphological average of selected gender-balanced young adult brains of the Common Marmoset monkey (Callithrix jacchus), derived from 3D reconstructions based on Nissl-stained serial sections.
   :documentation: `https://www.marmosetbrain.org/nencki_monash_template/ <https://www.marmosetbrain.org/nencki_monash_template/>`_
   :fullName: The Marmoset Nencki-Monash Template in Stereotaxic Coordinates
   :homepage: https://www.marmosetbrain.org/nencki_monash_template
   :howToCite: Majka, P., Bednarek, S., Chan, J. M., Jermakow, N., Liu, C., Saworska, G., Worthy, K. H., Silva, A. C., Wójcik, D. K., & Rosa, M. G. P. (2021). Histology-Based Average Template of the Marmoset Cortex With Probabilistic Localization of Cytoarchitectural Areas. NeuroImage, 226, 117625. https://doi.org/10.1016/j.neuroimage.2020.117625.
   :isVersionOf: `MarmosetNMT <https://openminds-documentation.readthedocs.io/en/latest/instance_libraries/commonCoordinateFrameworks.html#marmosetnmt>`_
   :nativeUnit: `millimeter <https://openminds-documentation.readthedocs.io/en/latest/instance_libraries/terminologies/unitOfMeasurement.html#millimeter>`_
   :relatedPublication: `https://doi.org/10.1016/j.neuroimage.2020.117625 <https://doi.org/10.1016/j.neuroimage.2020.117625>`_
   :releaseDate: 2021-02-01
   :shortName: Marmoset Nencki-Monash Template
   :usageCondition: `CC-BY-4.0 <https://openminds-documentation.readthedocs.io/en/latest/instance_libraries/licenses.html#cc-by-4-0>`_
   :versionIdentifier: v1
   :versionSpecification: This is the first version of average histology.

`BACK TO TOP <CommonCoordinateFrameworkVersions: MarmosetNMT library_>`_

------------

